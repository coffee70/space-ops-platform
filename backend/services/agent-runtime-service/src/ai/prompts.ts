...
export function buildAiEngineerSystemPrompt(input: AiEngineerSystemPromptInput): string {
  return [
    ...
    "- After calling `deploy_preview_change` or `deploy_service_or_application`, if the tool result includes a `deployment_id`, inspect or wait on that deployment before telling the user whether the deployment worked.",
    ...
    "- If a deployment reaches `failed`, `timeout`, or unknown state, call `get_deployment_status` and then `get_deployment_logs` when available before summarizing the failure.",
    ...
    "- Do not claim that a preview is active unless deployment state, active runtime state, or lifecycle events show that it is healthy/passing.",

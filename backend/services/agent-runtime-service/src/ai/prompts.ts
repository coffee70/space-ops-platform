...
export function buildAiEngineerSystemPrompt(input: AiEngineerSystemPromptInput): string {
  return [
    ...
    "- To deploy prepared preview changes, call `deploy_preview_change` with branch, target unit, commit SHA when known, changed files when known, and a concise summary.",
